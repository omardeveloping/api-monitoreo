...
from dashboard.models import EstadoVelocidadesVideo, EstadoVideo, Video
...
def _segmento_corresponde_a_turno(parsed: dict, turno) -> bool:
    ...
    inicio_turno, fin_turno = limites_turno(turno)
...
def _asegurar_cobertura_confiable(
    *,
    ...
    fecha_material = metadatos_nombre.get("fecha_material") or fecha_referencia
...
def _concatenar_mp4(rutas_mp4: list[str]) -> tuple[str, list[str]]:
    _asegurar_capacidad_temporal(
        sum(os.path.getsize(ruta) for ruta in rutas_mp4 if os.path.exists(ruta))
    )
...
def _normalizar_segmentos_a_mp4(segmentos: list[dict]) -> tuple[list[str], list[str]]:
    ...
    for segmento in segmentos:
        ...
        try:
            ...
            raise ValidationError(
                "No se pudo preparar un segmento raw para reconstrucción."
            ) from exc
...
def copiar_archivo_a_storage(
    ...
    *,
    ...
    if firma_esperada is not None and _firma_stat(_stat_archivo(origen_real)) != firma_esperada:
        raise ValidationError(
            "El archivo cambió antes de copiarse; espere a que termine la subida."
        )
...
def _estado_error_importacion(exc: Exception) -> str:
    tipo = _clasificar_error_importacion(exc)
    if tipo in {"estabilidad", "ruta", "espacio"}:
        return EstadoVideo.ERROR
    if isinstance(exc, ValidationError):
        return EstadoVideo.ERROR_PERMANENTE
    return EstadoVideo.ERROR
...
def marcar_video_con_error(video: Video | None, exc: Exception) -> None:
    ...
    video.estado = _estado_error_importacion(exc)
    ...
    video.ultimo_error = video.detalle_error
    video.save(
        update_fields=[
            "ruta_archivo",
            "estado",
...
def crear_video_pendiente_desde_ruta_servidor(
    ...
    *,
    ...
    return Video.objects.create(
        ...
        procesamiento_finalizado_en=None,
...
def _actualizar_campos_video(video: Video, validated_data: dict, manifest: dict, destino_rel: str) -> None:
    ...
    video.procesamiento_iniciado_en = timezone.now()
    video.procesamiento_finalizado_en = None
    video.tiempo_procesamiento_segundos = None
    video.estado_velocidades = EstadoVelocidadesVideo.PENDIENTE
    ...
    *,
    ...
    if (
        ...
        if video_obj and video_obj.pk != video_existente.pk:
            Video.objects.filter(pk=video_obj.pk).delete()
        return video_existente

    artifact_abs = manifest.get("artifact_abs")
    ...
    try:
        ruta_anterior = ""
        objetivo = video_obj
        if video_existente is not None and video_existente.pk:
            objetivo = video_existente
        if objetivo is not None and objetivo.pk:
            ruta_anterior = objetivo.ruta_archivo.name or ""
        ...
        if objetivo is None:
            ...
            _actualizar_campos_video(objetivo, validated_data, manifest, destino_rel)
            with transaction.atomic():
                _guardar_metadata_video(objetivo)
        try:
            procesar_video_subida(
                objetivo,
                objetivo.ruta_archivo,
                duracion_esperada=manifest["duracion_esperada"],
            )
            objetivo.mapa_segmentos = _ajustar_mapa_segmentos_a_duracion(
            ...
            default_storage.delete(destino_rel)
            marcar_video_con_error(objetivo, exc)
            raise
        if (
            video_obj
            and video_existente
            and video_obj.pk
            and video_existente.pk != video_obj.pk
        ):
            Video.objects.filter(pk=video_obj.pk).delete()
        if ruta_anterior and ruta_anterior != objetivo.ruta_archivo.name:
...
def listar_archivos_servidor(
    ...
    *,
    ...
    extensiones = extensiones or {".mp4", ".h264", ".grec"}
    rutas = []
    for raiz, _dirs, files in os.walk(base_dir_real):
        for nombre in files:
            ...
            rutas.append(ruta_relativa.replace(os.sep, "/"))

    rutas.sort()
    total = len(rutas)
    seleccion = rutas[offset : offset + limit] if limit else rutas[offset:]
    resultados = []
    for ruta_relativa in seleccion:
        ruta_absoluta = os.path.join(base_dir_real, ruta_relativa.replace("/", os.sep))
        try:
